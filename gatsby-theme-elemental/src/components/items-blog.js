...
class BlogItem extends React.Component{
	...
	componentDidMount(){
		this.color = window.getComputedStyle(this.textSecondary,null).getPropertyValue("color");
			...
			<div className="item col s12 m6">
				<div className="box">
					...
					<div className="content">
						<h3 className="text-primary"><a href={this.props.data.node.fields.slug} title={this.props.data.node.frontmatter.title}>{this.props.data.node.frontmatter.title}</a></h3>
						<p className="text-tertiary">{this.props.data.node.frontmatter.description}</p>
						<p className="date text-secondary" ref={c => this.textSecondary = c}>
